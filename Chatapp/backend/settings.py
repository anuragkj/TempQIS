...
class _SearchCommonSettings(BaseSettings):
    ...
    role_information: Optional[str] = Field(
    ...
    def set_datasource_settings(self) -> Self:
        try:
            if self.base_settings.datasource_type == "AzureCognitiveSearch":
                ...
                logging.warning("No datasource configuration found in the environment -- calls will be made to Azure OpenAI without grounding data.1")
                
            return self

        except ValidationError as e:
            logging.warning("No datasource configuration found in the environment -- calls will be made to Azure OpenAI without grounding data.2")
