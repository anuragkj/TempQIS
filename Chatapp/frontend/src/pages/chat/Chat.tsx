...
import { SquareRegular, ShieldLockRegular, ErrorCircleRegular, LightbulbRegular } from '@fluentui/react-icons'
...
import Connect_Logo from '../../assets/connect-logo.svg'
        ...
        while (true) {
          ...
          objects.forEach(obj => {
            try {
              ...
              if (!JSON.parse(obj).choices?.[0]?.messages?.[0].content){
          ...
          <div className={styles.chatContainer}>
            {!messages || messages.length < 1 ? (
              <Stack className={styles.chatEmptyState}>
                ...
                <img src={Connect_Logo} className={styles.chatIcon} aria-hidden="true" />
                {/* <h1 className={styles.chatEmptyStateTitle}>{ui?.chat_title}</h1> */}
                {/* <h1 className={styles.chatEmptyStateTitle}>Welcome to C.O.N.N.E.C.T A.I</h1> */}
                <div className={styles.typewriter}>
                  <h1>Welcome to C.O.N.N.E.C.T A.I</h1>

                </div>
                <h2 className={styles.chatEmptyStateSubtitle}>{ui?.chat_description}</h2>
                  <Stack horizontal className={styles.chatMessageTipContainer}>
                    <Stack.Item align="start">
                      <FontIcon aria-label="LightBulb" iconName="LightBulb" className={styles.chatMessageTipIcon} />
                    </Stack.Item>
                    <Stack.Item align="start">
                      <span className={styles.chatMessageTipText}>Tips to increase the answer accuracy while using the question templates below:</span>
                    </Stack.Item>
                    {/* <LightbulbRegular className={styles.chatMessageTipIcon}/> */}
                  </Stack>
                  <Stack  className={styles.chatMessageTipContainer}>
                    <Stack.Item className={styles.chatMessageTipListItem}>
                      <span>1. Keywords like <span className={styles.questioVariable}>--site--</span> should be replaced with the actual value without the '--' e.g. <span className={styles.questioVariable}>Jabel Ali</span>. </span>
                    </Stack.Item>
                    <Stack.Item className={styles.chatMessageTipListItem}>
                      <span>2. Any text enclosed within double dashes, such as <span className={styles.questioVariable}>--site--</span>, will prompt the chatbot to request additional information from you. </span>
                    </Stack.Item>
                    <Stack.Item className={styles.chatMessageTipListItem}>
                      <span>3. Using <span className={styles.questioVariable}>" "</span> can be used for an exact search.</span>
                    </Stack.Item>
                    
                  </Stack>
                <Stack horizontal>
                  <button className={styles.listItem} onClick={() => setselectedQuestions('SO')}>Service Operations</button>
                  <button className={styles.listItem} onClick={() => setselectedQuestions('PM')}>Project Management</button>
                </Stack>
                {selectedQuestions == 'PM' && 
                  <Stack horizontal wrap horizontalAlign='center'>
                    <button className={styles.listItem} onClick={() => {setquestionValue('QIS related to --logistics or shipping or customs-- in --site-- and summarize the learnings'); console.log("From Chat")}}>QIS related to <span className={styles.questioVariable}>--logistics / shipping / customs--</span> in <span className={styles.questioVariable}>--site--</span> and summarize the learnings</button>
                    <button className={styles.listItem} onClick={() => {setquestionValue('Provide me a list of QISs related to schedule or manpower or field services in --site-- and summarize the learnings')}}>List of QISs related to schedule or manpower or field services in <span className={styles.questioVariable}>--site--</span> and summarize the learnings</button>
                    <button className={styles.listItem} onClick={() => {setquestionValue('Provide unexpected findings for --fleet-- during --scope-- for --component--')}}>Provide unexpected findings for <span className={styles.questioVariable}>--fleet--</span> during <span className={styles.questioVariable}>--scope--</span> for <span className={styles.questioVariable}>--component--</span> </button>
                    <button className={styles.listItem} onClick={() => {setquestionValue('Provide list of QIS for issues related to "fuel quality" for --fleet--. I want to specifically search for exact term fuel quality')}}>Provide list of QIS for issues related to "<span className={styles.questioVariable}>--issue type e.g fuel quality--</span>" for <span className={styles.questioVariable}>--fleet--</span></button>
                    
                  </Stack>
                  
                }
                {selectedQuestions == 'SO' && 
                  <Stack horizontal wrap horizontalAlign='center'>
                    <button className={styles.listItem} onClick={() => {setquestionValue('Top 5 recurring safety issues reported for field service for --frame-- --compressor or turbine-- during --assembly or dissassembly or repair or preparation--')}}>Top <span className={styles.questioVariable}>--number--</span> recurring safety issues reported for field service for <span className={styles.questioVariable}>--frame--</span> <span className={styles.questioVariable}>--compressor / turbine--</span> during <span className={styles.questioVariable}>--assembly / dissassembly / repair / preparation--</span></button>
                    <button className={styles.listItem} onClick={() => {setquestionValue('Top 5 recurring learnings reported for field service for --frame-- --compressor or turbine-- during --assembly or dissassembly or repair or preparation--')}}>Top <span className={styles.questioVariable}>--number--</span> recurring learnings reported for field service for <span className={styles.questioVariable}>--frame--</span> <span className={styles.questioVariable}>--compressor / turbine--</span> during <span className={styles.questioVariable}>--assembly / dissassembly / repair / preparation--</span></button>
